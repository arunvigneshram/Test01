...
while True:
    ...
    for i, connection in enumerate(connections):
        ...
        for item in charger_data:
            if item["topic"] == "evse/state":
                ice61851_state = item["payload"]["ice61851_state"]
        ...
        if ice61851_state == 1 and charger_state == 2 and error_state == 0 and phase_state:
            ready_state = "READY"
        else:
            ready_state = "WAITING"
        ...
        chargers_status_df = chargers_status_df.append({"charger_id": config.chargers[i]["id"],
    ...
    # chargers_status_df.set_index("charger_id", inplace=True)
    print(chargers_status_df)

    # Save dataframe as pickle
    chargers_status_df.to_pickle("./chargers_status_df.pkl")
